...
const ProductDetailsPage = ({ productId }) => {
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            {showLightbox && (
                <div className="fixed inset-0 bg-white md:bg-gray-900/60 md:backdrop-blur-[3px] z-[100] flex items-center justify-center p-0 md:p-8" onClick={() => setShowLightbox(false)}>
                    <div className="relative w-full h-full md:w-auto md:h-auto md:inline-block flex items-center justify-center bg-white md:bg-transparent" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => setShowLightbox(false)} className="absolute top-4 right-4 z-50 text-black md:bg-white md:p-2 rounded-full">X</button>
                        <div className="relative w-full h-full md:w-auto md:h-auto flex items-center justify-center">
                            <img src={product.images[selectedImage]} alt={product.name} className="w-full h-auto max-h-full md:h-[95vh] md:w-auto md:max-w-[90vw] object-contain md:rounded md:shadow-2xl" />
                        </div>
                    </div>
                </div>
            )}
