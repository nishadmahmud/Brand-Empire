...
export async function getProducts(page = 1, category_id = 0, subcategory_id = 0) {
    let url = `${process.env.NEXT_PUBLIC_API}/v1/publlic/products/userwise?user_id=${process.env.NEXT_PUBLIC_USER_ID}&page=${page}&per_page=20&category_id=${category_id}`;
 ...
 * @returns {Promise<Object>} Products data
 */
export async function getProductsBySubcategory(page = 1, category_id, subcategory_id) {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API}/v1/publlic/products/userwise?user_id=${process.env.NEXT_PUBLIC_USER_ID}&page=${page}&per_page=20&category_id=${category_id}&subcategory_id=${subcategory_id}`
    );
    return await res.json();
}
