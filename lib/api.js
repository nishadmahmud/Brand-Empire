...
/**
 * Filter products by attribute value IDs (with caching)
 * @param {number[]} attributeValueIds - Array of attribute value IDs to filter by
 * @param {number} page - Page number (default: 1)
 * @returns {Promise<Object>} Filtered products data
 */
export async function filterProductsByAttributes(attributeValueIds = [], page = 1) {
    const cacheKey = `filter_products_${attributeValueIds.sort().join('_')}_page_${page}`;
    return getCachedOrFetch(cacheKey, async () => {
...
/**
 * Get all studios (authenticated)
 * @returns {Promise<Object>} All studios data
 */
export async function getStudioList() {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API}/studio-list`
    );
    return await res.json();
}
